...
module.exports = (client) => {
    ...
    client.on("guildMemberAdd", async (member) => {
        try {
            ...
            // 4. NOTIFICATIONS ET TAG
            async function startNotifStep() {
                const notifMenu = new StringSelectMenuBuilder()
                    .setCustomId("select_notifs")
                    .setPlaceholder("Sélectionne tes préférences de notifications...")
                    .setMinValues(0)
                    .setMaxValues(6)
                    .addOptions([
                        { label: "Annonces Officielle", value: "annonces", description: "Actualités majeures de la structure" },
                        { label: "Animations / Événements", value: "anim", description: "Soirées jeux, tournois et animations" },
                        { label: "Sondages", value: "sondage", description: "Participer aux choix de la communauté" },
                        { label: "WebTV / Streams", value: "webtv", description: "Alertes lives de nos créateurs" },
                        { label: "Partenariats", value: "partenaire", description: "Offres et concours partenaires" },
                        { label: "Réseaux Sociaux", value: "reseaux", description: "Nouveaux posts X, TikTok, YouTube" }
                    ]);

                const msgNotif = await channel.send({
                    content: `### 🔔 Étape 3/4 — Quels rôles de notifications souhaites-tu recevoir ?`,
                    components: [new ActionRowBuilder().addComponents(notifMenu)]
                });
                ...
                notifCollector.on("collect", async (i) => {
                    ...
                    i.values.forEach(val => {
                        if (CONFIG.ROLES_NOTIFS[val]) {
                            selectedNotifs.push(...CONFIG.ROLES_NOTIFS[val]);
                        }
                    });

                    await webhookCEO.send({ content: `✅ Vos préférences de notifications ont été enregistrées avec succès !` });
            ...
            // ÉTAPE FINALE : TAG ET VALIDATION
            async function startTagStep() {
                const tagRow = new ActionRowBuilder().addComponents(
                    new ButtonBuilder().setCustomId("tag_yes").setLabel("Oui, ajouter [HLR]").setStyle(ButtonStyle.Success).setEmoji("🏷️"),
                    new ButtonBuilder().setCustomId("tag_no").setLabel("Non merci").setStyle(ButtonStyle.Secondary)
                );

                const msgTag = await channel.send({
                    content: `### 🏷️ Étape 4/4 — Souhaites-tu porter le tag officiel **HLR** devant ton pseudo sur le serveur ?`,
                    components: [tagRow]
                });
                ...
                tagCollector.on("collect", async (i) => {
                    ...
                    shouldAddTag = (i.customId === "tag_yes");
